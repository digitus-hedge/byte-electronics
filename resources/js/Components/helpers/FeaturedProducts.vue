...
<template>
    <div class="card mb-2 shadow-sm">
        <div class="card-header d-flex justify-content-between align-items-center"
            ...
            <span class="px-2 py-1 rounded" style="background: #555; color: #fff; font-size: 14px;">
                {{ brand }}
            </span>
        </div>

        <Link v-for="(item, index) in products" :key="index"
            :href="`/${item.category_slug}/${item.subcategory_slug}/${item.slug}`"
            class="d-flex border-bottom p-3 align-items-start text-decoration-none"
            style="color:inherit; cursor:pointer;">
            <img :src="`/uploads/products/${item.file_name}`" @error="$event.target.src = fallbackImage"
                alt="product image" class="me-3" style="width:80px; height:80px; object-fit:contain;" />
